...
public class UserResource {
	...
	public ResponseEntity<List<User>> findAll() {
		//User u = new User(1L, "Mike", 123456789, "Aveiro", 912375483);
	...
	public ResponseEntity<List<User>> findByNome(@PathVariable String nome) {
		List<User> list = service.findByNome(nome);
		return ResponseEntity.ok().body(list);
	}
	...
	public ResponseEntity<User> insert(@RequestBody User obj) {
		...
		// forma 1 - retornar 200
		// return ResponseEntity.ok().body(obj);
		
		// forma 2 - retornar 201
	...
	public ResponseEntity<User> delete(@PathVariable Long id) {
		service.delete(id);
		return ResponseEntity.noContent().build(); //noContente - resposta vazia - 204
	}
